...
# Save the result
os.makedirs('Lesson6/result', exist_ok=True)


# And finally we plot the images (with the detected document corners)
plt.subplot(121), plt.imshow(out)
plt.subplot(122), plt.imshow(cornerness)
cv2.imwrite('Lesson6/result/sheet_corners.jpg', cv2.cvtColor(out, cv2.COLOR_RGB2BGR))
plt.show()

'''
- Does it matter whether the picture has been taken by a 1Mpx camera or a 12Mpx camera? How?
  якщо роздільна здатність буде дуже велика, то ми повиділяємо ключові точки не лише на краях листа, але й, наприклад, на контурах символів надрукованого тексту. Тому, якщо ми захочемо все ж знайти краї листа, то легше вьсого заблюрити, або стиснути оригінальне зображення із роздільною здатністю і тоді вже відшуковувати кути. При заблюреному зображенні нам довдедеться при аналізі корнерності використовувати більший кернел. І, взагалі, це буде не дуже ефективний метод, краще стиснути зображення, знайти краї, а потім, при потребі, перерахувати координати знайдених кутів для зображенжня високої роздільної здатності з урахуванням коефіцієнту стискання.
-   If we increased the resolution of the camera, what would you change in the current algorithm?
в принципі, я вже дав відповідь в попередньому питанні. Коротко, то додам ось такий рядок:

transformed_img = cv.resize(img, None, fx=0.083, fy=0.083, interpolation=cv.INTER_AREA)

і далі вжепрацюватиму із цим зображенням, аналізуючи корнерість і та відшукуючи ключові точки
'''
